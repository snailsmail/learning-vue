    ...
    <div>
      <ul>
        <li v-for="(category, index) in categoryList" :key="category.id"
          ...
          <a href="javascript:void(0);" :class="{categorySelected: currentCategory == category.id}">
            {{category.name}}
          </a>
        </li>
      </ul>
  ...
  export default {
    ...
    methods: {
      changeCategoryHandle (id) {
        this.$router.push({name:'phone.list', params: {categoryId: id}});
